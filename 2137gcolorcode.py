...
import numpy as np
import pandas as pd
green_red = pd.read_csv('data.csv', delimiter = ',', usecols = (6, 16), skiprows = np.arange(52, 264))
ri_and_scale = pd.read_csv('data.csv', delimiter = ',', usecols = (5, 8), skiprows = np.arange(52, 264))
...
plt.xlabel('g-r')
plt.ylabel('r-i')
plt.xticks(np.arange(int(np.min(x)), int(np.max(x)) + 1, 1))
plt.title('r-i v.s. g-r Color Filters for Plate 2137 (nQSOs = 51)')
plt.savefig('2137_gcolor-color_graph.png')
